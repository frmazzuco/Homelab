...
## Ajustes para clawdbot/moltbot
- Mantenha `allowed_ssh_cidrs` restrito ao seu IP publico.
- Se precisar expor uma porta especifica para o bot, use `allowed_tcp_ports` e `allowed_tcp_cidrs`.
- O firewall do SO so libera portas definidas em `allowed_tcp_ports`.
- Se o bot e o browser tool rodam na mesma VM, nao exponha as portas do browser (use loopback).
- Para acessar a UI do bot com seguranca, prefira um tunnel SSH:
